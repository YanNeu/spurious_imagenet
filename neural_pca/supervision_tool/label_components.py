...
def label_vis(visualisation_dir, target_class, save_suffix=''):
    class_str = f'00{target_class}_{imagenet_label2class[target_class]}'
    if target_class >= 100:
        class_str = class_str[2:]
    elif target_class >= 10:
        class_str = class_str[1:]
    vis_dir = f'{visualisation_dir}/{class_str}'

    buttons = widgets.ToggleButtons(options=['spurious', 'not spurious', 'inconsistent'])

    b_prev = widgets.Button(description='Previous')
    b_next = widgets.Button(description='Next')
    
    def save_results(res):
        pc_idcs = list(res.keys())
        ...
        for pc_idx in pc_idcs:
            components[res[pc_idx]].append(pc_idx)
        
        np.save(f'{vis_dir}/vis_labels_{save_suffix}.npy', components)
    vis_files = {}
    ...
    for vis_file in os.listdir(vis_dir):
        if vis_file[-4:] != '.png':
            continue
        pc_idx = int(vis_file[20:-4])
        vis_files[pc_idx] = vis_file
        results[pc_idx] = None

    pc_idcs = list(results.keys())
    current_dict = {
        'current': 0, 
        'pc_idcs':pc_idcs,
        'files':vis_files,
    ...
    def run_components(current_dict):
        img_path = f'{current_dict["dir"]}/{current_dict["files"][current_dict["pc_idcs"][current_dict["current"]]]}'
        vis_img = display(Image(img_path), buttons, b_prev, b_next)


    def on_next(b):
        if current_dict['current'] < len(current_dict['pc_idcs'])-1:
            results[current_dict['pc_idcs'][current_dict['current']]] = buttons.value
            current_dict['current'] += 1
            clear_output(wait=True)
            run_components(current_dict)
        else:
            results[current_dict['pc_idcs'][current_dict['current']]] = buttons.value
            clear_output(wait=True)
            print(results)
            save_results(results)
        print(current_dict['current'], '/', len(current_dict['pc_idcs'])-1)
    
    def on_prev(b):
        if current_dict['current'] > 0:
            current_dict['current'] -= 1
            clear_output(wait=True)
            run_components(current_dict)
    ...
    return run_components, current_dict
