...
def calculate_spurious_score(in_loader, out_loader, device, model, tpr_threshold=None):
...
def eval_auc(device, device_ids, bs):
    with open(os.path.join(dataset_dir, 'timestamp.txt'), 'r') as f:
        line = f.readline()
        dataset_timestamp = int(line.rstrip())

    mean_scores = np.zeros(len(model_names))
    joint_scores = np.zeros(len(model_names))
    class_scores = []
    print('Evaluating models...')
    with tqdm(total=len(model_names)) as pbar:
        for i, model_name in enumerate(model_names):
            ...
            mean_scores[i] = mean_auc_score
            joint_scores[i] = joint_auc_score
            class_scores.append(model_class_auc_scores)
